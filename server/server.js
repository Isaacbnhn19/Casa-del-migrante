...
app.post("/api/donations/manual", requireAuth, async (req, res) => {
  try {
    ...
    if (!name || !amount || amount <= 0 || !payment_method) {
      return res.status(400).json({ error: "Datos incompletos para la donación manual" });
    }

    if (!["transferencia", "tarjeta"].includes(payment_method)) {
      return res.status(400).json({ error: "Método de pago no válido" });
    }

    const phone = "";
    const email = "registro.manual@casaranzahuer.local";
    const message = "Registro manual desde panel administrativo";
    ...
    console.error("❌ /api/donations/manual POST error:", err);
    res.status(500).json({ error: "Error registrando la donación manual" });
  }
});
